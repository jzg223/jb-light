...
bt.soft = {
			...
			content:
				...
				'专业版</span></p>\
                    ...
                    <div class="libPay-menu-type lib_ltd" >\
                        <p class="vip_text"><span class="glyphicon glyphicon-vip"></span><span style="margin-left:8px">' +
				bt.os +
				'企业版</span></p>\
				<img class="vip_img" style="width: 118px;margin-top: 21px;margin-bottom: 3px;display: none;" src="/static/images/svg/Linux_enterprise.svg">\
				...
				(config.plugin
					...
					: '<div class="libPay-menu-type lib_ops" >\
                        <p class="vip_text"><span class="glyphicon glyphicon-vip"></span><span style="margin-left:8px">企业运维托管</span></p>\
                        <img class="vip_img" style="width: 118px;margin-top: 21px;margin-bottom: 3px;display: none;" src="/static/images/svg/enterprise_operation_and_maintenance_services.svg">\
                ...
                <div id="pay_product_view">\
                    ...
                    <div class="libPay-layer-item">\
                        <div class="libPay-line-item proS" id="libPay-theme-tips">\
												...
												<span></span></div>\
                        <div class="libPay-line-item proTname" style="margin-bottom:16px">产品周期</div>\
				...
				bt.send('get_coupons','auth/get_coupons',{},function(res_coupons){
					bt.send('get_plugin_remarks', 'auth/get_plugin_remarks', {}, function (res) {
						...
						if(typeof res.activity_list != 'undefined' && res.activity_list.length > 0){
							...
							$('#libPay-theme-tips span').html(res.activity_list[0])
						}else{
            ...
            var ops_obj = {
              ...
              rlist: [
                ...
                'CPU内存占用100%处理',
						...
						if(config.plugin && hide_plugin){
							$('.libPay-product-content .libPay-menu .lib_ver').before('<div class="libPay-menu-type lib_ops" ><p><span class="glyphicon glyphicon-vip"></span><span style="margin-left:8px">企业运维托管</span></p>\<p>适用于无专业技术、需技术服务的企业</p></div>')
							$('.libPay-product-content .libPay-menu').addClass('.is_ops')
							arry.push(ops_obj);
						}
	...
	set_pay_status_icon:function(){
		...
		bt.send('get_pd', 'ajax/get_pd', {}, function (res) {
			...
			if(!is_pay){
				// 其他界面
				proHTML = '<span onclick="bt.soft.updata_ltd()" style="margin-right:4px" title="点击升级到企业版">免费版</span>'
				// 免费版按钮
				// proHTML_button = '<div class="product-buy" style="margin-right:0;display:flex;align-items:center"><div onclick="product_recommend.pay_product_sign(\'ltd\',80,\'ltd\')" class="immediately-tip"><span class="recommend-immediate-icon"></span><span style="color:#F2B53A;line-height:18.2px">安全、高效，让您更安心！</span></div><button type="button" class="btn btn-xs immediate-experience" style="vertical-align: 1px;" onclick="product_recommend.pay_product_sign(\'ltd\',80,\'ltd\')">立即体验</button></div>'
				...
				$('.pos-box .authState .product-buy').remove()
				if(!tab.children().eq(-1).hasClass('has_pay')){
					tab.append(res[0])
					tab.children().eq(-1).removeClass('bt'+advanced).addClass('has_pay')
				}else{
					tab.children().eq(-1).remove()
					tab.append(res[0])
					tab.children().eq(-1).removeClass('bt'+advanced).addClass('has_pay')
			...
			if(tab.children(0).text().includes('免费版')){
				var str = $('.pos-box .authState .product_version').text().replace('免费版','')
				tab.children().eq(0).text(str)
			}
			tab.prepend(proHTML);
			// 首页替换
	...
	get_product_change: function (idx, btype) {
		...
		$('.vip_text').show();
		$('.vip_img').hide();
		_obj.find('.vip_img').show();
		_obj.find('.vip_text').hide();
		var _data = _obj.data('data');
		var getImg = function (name){
			if(name.title=='企业版'){
				return '<img src="/static/images/svg/products_log1.svg">'
			}else if(name.title=='专业版'){
				return '<img src="/static/images/svg/protect_logo.svg">'
			}else {
				return '<img src="/static/images/svg/enterprise_operation.svg">'
			}
		}
		var getImg1 = function (name){
			if(name.title=='企业版'){
				return '<img src="/static/images/svg/enterprise_privileges.svg">'
			} else if(name.title=='专业版'){
				return '专业版特权：'
			}else {
				return '<img src="/static/images/svg/enterprise_op.svg">'
			}
		}
		if (btype !== 'ver') {
			var _html = '',
			...
			if (btype !== 'plugin') {
                ...
                <div class="pro-price-herf"><a class="privilege_contrast" href="javascript:;"><img src="/static/images/svg/viewing_privileges.svg"></a></div>\
                </div>';
			} else {
				_html =
					'<div class="pro-left-introduce ' +
			...
			voucherIntroduce =
				'<div class="pro-left-introduce ' +
				_data.type +
				'"><div class="pro-left-title"><div><span><img src="/static/images/svg/discount_coupon.svg" </span></div><span>抵扣券授权</span></div>\
			<div class="pro-left-list">\
	...
	create_pay_code: function (idx) {
		...
		}else if(_coupon.name.indexOf('0.99元7天体验卷')!=-1){
			var $children = $('.pay-pro-cycle').children()
			var $num_children = $('#libPay-theme-nums').children()
			$.each($num_children,function (index,item) {
				if(index != 0){
